...
struct Home: View {
                    ...
                    HStack(spacing: 12) {
                        ForEach(row, id:\.self) {item in
                            Button {
                                ...
                                Text(item.rawValue)
                                    .font(.system(size: 32))
                                    .frame(width: self.buttonWdith(item: item), height:
                                            self.buttonHeight())
                                    .background(item.buttonColor)
                                    .clipShape(.rect(cornerRadius: self.buttonWdith(item: item) / 2))
                                    .foregroundColor(.white)
    ...
    //Button Width
    func buttonWdith(item: CalculatorButtons) -> CGFloat {
        if item == .zero {
    ...
    func didTap(button: CalculatorButtons) {
        ...
        default:
            ...
            if self.displayValue == "0" {
                ...
                displayValue = "\(self.displayValue)\(number)"
            }
